...
const features = [
  {
    ...
    description:
      'Collect rent on time with reminders, autopay, and real-time payment tracking for every unit.',
    tag: 'Finance',
    tagColor: 'bg-sky-100 text-sky-700 dark:bg-sky-500/20 dark:text-sky-300',
    gradient: 'from-sky-50/60 to-blue-50/30 dark:from-sky-950/30 dark:to-blue-950/20',
    border: 'border-sky-200/50 dark:border-sky-500/20',
    bars: [
      { w: 'w-3/4', opacity: 'opacity-40' },
      { w: 'w-full', opacity: 'opacity-25' },
      { w: 'w-2/3', opacity: 'opacity-35' },
    ],
  },
  {
    title: 'Maintenance Workflows',
    description:
      'Route tenant requests to vendors fast, monitor status, and keep owners informed without extra admin.',
    tag: 'Operations',
    tagColor: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-300',
    gradient: 'from-emerald-50/60 to-teal-50/30 dark:from-emerald-950/30 dark:to-teal-950/20',
    border: 'border-emerald-200/50 dark:border-emerald-500/20',
    bars: [
      { w: 'w-full', opacity: 'opacity-35' },
      { w: 'w-4/5', opacity: 'opacity-25' },
  ...
  },
  {
    ...
    description:
      'Track occupancy, delinquency, and NOI with clean dashboards built for real estate operations.',
    tag: 'Insights',
    tagColor: 'bg-violet-100 text-violet-700 dark:bg-violet-500/20 dark:text-violet-300',
    gradient: 'from-violet-50/60 to-purple-50/30 dark:from-violet-950/30 dark:to-purple-950/20',
    border: 'border-violet-200/50 dark:border-violet-500/20',
    bars: [
      { w: 'w-5/6', opacity: 'opacity-40' },
      { w: 'w-full', opacity: 'opacity-30' },
      { w: 'w-2/3', opacity: 'opacity-25' },
    ],
  },
  {
    title: 'Owner & Tenant Portals',
    description:
      'Give stakeholders secure access to statements, leases, and updates from one shared platform.',
    tag: 'Access',
    tagColor: 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-300',
    gradient: 'from-amber-50/60 to-orange-50/30 dark:from-amber-950/30 dark:to-orange-950/20',
    ...
    >
        ...
        <div className="relative mx-auto flex w-full max-w-6xl flex-col items-center px-6 pb-20 pt-28 md:pt-36">
          <div className="anim-fade-1 mb-7">
            <span className="inline-flex items-center gap-2 rounded-full border border-border bg-background/90 px-4 py-1.5 text-xs font-medium text-muted-foreground shadow-sm backdrop-blur-sm">
              ...
              Built for modern real estate managers
            </span>
          </div>

          <h1
            className="anim-fade-2 max-w-4xl text-center text-5xl font-normal leading-[1.12] tracking-tight text-foreground md:text-7xl"
          ...
          <p className="anim-fade-3 mt-6 max-w-xl text-center text-base leading-relaxed text-muted-foreground md:text-lg">
            HomiQ centralizes leasing, rent collection, maintenance, and
            reporting so property teams can scale faster while delivering
          ...
          <div className="anim-fade-4 mt-8 flex flex-wrap items-center justify-center gap-3">
            ...
            <Button size="lg" variant="outline" className="px-7 bg-background/70 backdrop-blur-sm">
              Contact Sales
            </Button>
          </div>
            ...
            <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-2xl">
              <div className="flex items-center gap-3 border-b border-border bg-muted/70 px-4 py-3">
                ...
                <div className="mx-auto flex items-center gap-1.5 rounded-md border border-border bg-background px-3 py-1 text-xs text-muted-foreground shadow-sm">
                  <svg className="h-2.5 w-2.5 text-muted-foreground/60" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                ...
                <aside className="hidden border-r border-border bg-muted/40 p-4 md:block">
                  <div className="mb-5 flex items-center gap-2.5">
                    <div className="flex h-6 w-6 items-center justify-center rounded-md bg-primary">
                      <svg className="h-3.5 w-3.5 text-primary-foreground" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z" />
                      </svg>
                    </div>
                    <span className="text-sm font-semibold text-foreground">HomiQ</span>
                  </div>
                  ...
                  <nav className="space-y-0.5">
                    {[
                      ...
                      { label: 'Lease Renewals', active: false },
                    ].map((item) => (
                      <div
                        key={item.label}
                  ...
                  <nav className="space-y-0.5">
                    {['Financials', 'Occupancy'].map((label) => (
                      <div key={label} className="rounded-md px-3 py-2 text-xs font-medium text-muted-foreground">
                        {label}
                      </div>
                    ))}
                  </nav>
                ...
                <div className="p-5 md:p-6">
                  <div className="mb-5 flex items-start justify-between gap-4">
                    <div>
                      ...
                      <p className="mt-1 text-2xl font-semibold text-foreground">$1,248,900</p>
                      <p className="text-xs text-emerald-600 dark:text-emerald-400">↑ 8.2% from last period</p>
                    </div>
                    <div className="flex gap-2">
                      <div className="rounded-lg border border-border bg-muted px-3 py-2 text-center">
                        <p className="text-[10px] font-medium text-muted-foreground">Units</p>
                        <p className="text-sm font-semibold text-foreground">128</p>
                      </div>
                      <div className="rounded-lg border border-border bg-muted px-3 py-2 text-center">
                        <p className="text-[10px] font-medium text-muted-foreground">Occupancy</p>
                        <p className="text-sm font-semibold text-emerald-600 dark:text-emerald-400">94%</p>
                      </div>
                      <div className="rounded-lg border border-border bg-muted px-3 py-2 text-center">
                  ...
                  <div className="relative rounded-xl border border-border bg-muted/50 p-4">
                    <div className="mb-3 flex items-center justify-between">
                      <p className="text-xs font-medium text-foreground">Revenue · All Products</p>
                    ...
                    <div className="relative h-32">
                      ...
                      >
                        {[32, 64, 96].map((y) => (
                          <line key={y} x1="0" y1={y} x2="620" y2={y} stroke="hsl(var(--foreground) / 0.05)" strokeWidth="1" />
                        ))}
                        ...
                        <circle cx="355" cy="58" r="4" fill="hsl(var(--background))" stroke="hsl(var(--foreground) / 0.8)" strokeWidth="2" className="stat-pulse" />
                      ...
                      <div className="absolute left-[55%] top-0 -translate-x-1/2 -translate-y-1 rounded-md border border-border bg-card px-2 py-1 text-[10px] shadow-sm">
                        <p className="font-semibold text-foreground">$218,400</p>
      ...
      <section className="border-y border-border py-12">
        <p className="mb-8 text-center text-xs font-semibold uppercase tracking-widest text-muted-foreground">
          Trusted by property operators, asset managers &amp; owner groups
        </p>
        ...
        <div className="mx-auto max-w-6xl px-6">
          <div className="mx-auto max-w-3xl space-y-4 text-center">
            ...
            <p className="text-base leading-relaxed text-muted-foreground md:text-lg">
              Manage properties, residents, owners, and finances from one place
              with workflows designed for teams that operate at scale.
            </p>
          </div>
          ...
          <div className="mt-14 grid gap-5 md:grid-cols-2">
            {features.map((feature) => (
              ...
              >
                <div className="mb-4">
                  <span className={`inline-block rounded-full px-2.5 py-0.5 text-xs font-semibold ${feature.tagColor}`}>
                    {feature.tag}
                  </span>
                </div>
                ...
                <div className="mt-6 space-y-2">
                  {feature.bars.map((bar, i) => (
                    <div key={i} className="flex items-center gap-2">
                      <div className={`h-1.5 ${bar.w} rounded-full bg-foreground/20 ${bar.opacity}`} />
                    </div>
                  ))}
                </div>
      ...
      <section className="relative overflow-hidden py-28 md:py-36">
        ...
        <div className="relative mx-auto max-w-3xl px-6 text-center">
          ...
          <p className="mt-5 text-base leading-relaxed text-muted-foreground md:text-lg">
            Join hundreds of property managers who streamlined their operations
            with HomiQ. Start free, scale as you grow.
          </p>
          <div className="mt-9 flex flex-wrap items-center justify-center gap-3">
            <Button size="lg" className="px-8 shadow-md">
              Start Free Trial
            </Button>
            <Button size="lg" variant="outline" className="px-8 bg-background/70 backdrop-blur-sm">
              Book a Demo
            </Button>
          </div>
        </div>
