...
export const { handlers, signIn, signOut, auth } = NextAuth({
  ...
  callbacks: {
    async jwt({ token, user }) {
      ...
      if (
        Date.now() < refreshTokenValidityInMs &&
    ...
    async session({ session, token }) {
      ...
      // Surface the backend access token so server-side fetches can authenticate
      // tenant-scoped requests to the gateway.
