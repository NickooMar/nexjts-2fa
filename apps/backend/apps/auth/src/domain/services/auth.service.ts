...
import { TenantProxy } from 'apps/user/src/infrastructure/external/tenant.proxy';
...
export class AuthService extends AuthServiceAbstract {
  constructor(
    ...
    private readonly tenantProxy: TenantProxy,
  ...
  signup(input: SignupRequestDto) {
    return from(this.userProxy.findByEmail(input.email)).pipe(
      switchMap((existingUser) => {
        ...
        if (!existingUser) {
          // Signup only creates the identity. Organizations are created (or
          // joined via invitation) later, from the dashboard.
      ...
      switchMap((payload) =>
        from(this.userProxy.findById(payload.sub)).pipe(
          switchMap((user) => {
            ...
            // Keep the user in whichever org their refresh token was scoped to.
            return from(this.generateTokens(user, payload.tenantId)).pipe(
              map((tokens) => {
                console.log(
  ...
  /**
   * Create a new organization for an already-authenticated user: tenant +
   * owner membership, then re-issued tokens scoped to the new org so the
   * session lands inside it immediately.
   */
  createOrganization(
    userId: string,
    name: string,
    ...
    return from(
      this.provisionOrganization(userId, name, website, phone, country),
    ).pipe(
      catchError((error) => throwError(() => new RpcException(error.message))),
    );
  }
  ...
  private async provisionOrganization(
    ...
    const user = await firstValueFrom(
      this.userProxy
        .findById(userId)
        .pipe(catchError(() => of(null as User | null))),
    );
    if (!user) throw new RpcException('user_not_found');

    // First org becomes the login default.
    const existingPrimary = await firstValueFrom(
    ...
    const tenant = await firstValueFrom(
      this.tenantProxy.create({ name, ownerId: userId, website, phone, country }),
    );

    await firstValueFrom(
      this.membershipProxy.create({
        userId,
    ...
    const tokens = await this.generateTokens(user, String(tenant._id));
   ...
   * Membership is validated first — a user can never switch into an org they
   * are not a member of.
   */
  switchTenant(userId: string, tenantId: string): Observable<any> {
    return from(
      this.membershipProxy.findByUserAndTenant(userId, tenantId),
    ).pipe(
      switchMap((membership) => {
        if (!membership) {
          return throwError(() => new RpcException('not_a_member'));
        }
        return from(this.userProxy.findById(userId)).pipe(
          switchMap((user) => {
            if (!user) {
              return throwError(() => new RpcException('user_not_found'));
            }
            return from(this.generateTokens(user, tenantId)).pipe(
              map((tokens) => ({ success: true, tokens })),
            );
          }),
        );
      }),
  ...
  /**
   * Resolve the membership a token should be scoped to: the requested tenant
   * (when the user is a member) or the user's primary organization.
   */
  private async resolveMembership(userId: string, tenantId?: string) {
    if (tenantId) {
      const scoped = await firstValueFrom(
        this.membershipProxy
          .findByUserAndTenant(String(userId), tenantId)
          .pipe(catchError(() => of(null))),
      );
    ...
    return firstValueFrom(
      this.membershipProxy
        .findPrimaryForUser(String(userId))
        .pipe(catchError(() => of(null))),
    );
  }
  ...
  private async generateTokens(
    ...
    // Resolve which org this token is scoped to, then load its tenant so the
    // access token carries everything the gateway needs to route requests.
    const membership = await this.resolveMembership(user._id, scopedTenantId);
    const tenantId = membership ? String(membership.tenantId) : null;

    const tenant = tenantId
      ? await firstValueFrom(
          this.tenantProxy.findById(tenantId).pipe(catchError(() => of(null))),
        )
      : null;
