...
export class AuthController {
  ...
  @MessagePattern({ cmd: AuthPatterns.SWITCH_TENANT })
  switchTenant(payload: {
    userId: string;
    tenantId: string;
  }): Observable<any> {
    return this.authService.switchTenant(payload.userId, payload.tenantId);
  }

  @MessagePattern({ cmd: AuthPatterns.CREATE_ORGANIZATION })
  ...
  @MessagePattern({ cmd: AuthPatterns.REFRESH_TOKEN })
